...
The baseline YOLOv8-Seg small model established the main reference point for all
...
University R29. These images were split into 32 training images, 7 validation
...
retrained model detected buttons in more replayed frames (295 vs.\ 237), and
more of those detections could be paired with valid depth estimates (288 vs.\ 231).
The retrained model also achieved a higher mean target confidence (0.8422 vs.\ 0.8184).
Distance-related statistics remained broadly comparable: the mean valid-target distance
stayed close to the baseline (0.6776~m vs.\ 0.6975~m), while the median distance
increased slightly (0.5843~m vs.\ 0.5594~m) and the 90th-percentile distance also
increased slightly (0.8310~m vs.\ 0.7955~m). Maximum-distance values are not reported
here because the farthest outliers included implausible depth estimates that did
not correspond to real button observations. Qualitative inspection of matched replay
frames also showed individual cases in which the retrained model detected buttons at
substantially larger distances than the baseline model. For example, one repeatedly
inspected button was detected at approximately 0.75~m rather than about 0.35~m.
However, these case-specific improvements should be interpreted separately from the
aggregate replay statistics, which indicate only modest overall distance improvement.
This replay result complements the held-out test-set results by showing that
\gls{adc}-based retraining improved deployment robustness and confidence on the actual
target environment, while maintaining broadly comparable overall distance behavior.
...
observed during live testing with the mobile cart in HAN University R29. A
