...
the baseline configuration. Most sampled Stage 1 trials did not produce
meaningful mask performance and therefore did not progress to later stages.
Figure~\ref{fig:stage1_mask_map} focuses on the small subset of successful Stage 1
trials and shows their validation mask $\mathrm{mAP}_{50\text{:}95}$ trajectories.
These runs converged into a similar performance band, which justified carrying
only the best-performing candidates forward to Stage 2. The unsuccessful
zero-valued trials still count as valid tuning outcomes because they identify
poor regions of the hyperparameter space, but they are not shown in this figure
in order to keep the comparison readable.
...
configurations. In contrast to Stage 1, Stage 2 produced a larger number of
usable configurations because the search was restricted to the top Stage 1
candidates and only varied image size and batch size. The logged results show
that eight out of sixteen unique Stage 2 configurations achieved non-zero mask
$\mathrm{mAP}_{50\text{:}95}$ values. Most successful runs were observed at an
input resolution of 640 pixels, while the main successful 768-pixel exception was
\texttt{cfg1\_b16\_sz768}, which also produced the best overall Stage 2 result
...
\begin{figure}[htbp]
    ...
    \caption{Validation mask $\mathrm{mAP}_{50\text{:}95}$ for the successful unique Stage 2 capacity-tuning configurations. The \texttt{cfg1\_b16\_sz768} setting achieved the strongest validation performance and was selected for Stage 3 refinement.}
