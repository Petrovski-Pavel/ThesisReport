...
run was recorded as a \gls{ros2} bag so that the baseline and \gls{adc}-fine-tuned
...
\begin{table}[htbp]
    \centering
    \caption{Replay-based comparison on the same recorded \gls{ros2} bag from the university elevator environment.}
...
integrating the models into the full \gls{ros2} pipeline.
...
observed during live testing with the mobile cart in HAN University R29. The
illustrated frames come from the same \gls{ros2} bag sequence used for the
