...
well as a small subset prioritized by low mean \gls{iou} or elevated
false-positive and false-negative counts from a baseline model, mainly
highlighted difficult yet valid samples, such as oblique viewpoints, small
targets, and visually cluttered panels. The manual review further indicated
that the annotations in these low-metric images generally matched the visible
...
\begin{figure}[htbp]
    ...
    \caption{Validation mask $\mathrm{mAP}_{50\text{:}95}$ for the successful Stage 1 hyperparameter trials. Only a limited subset of the Stage 1 configurations produced stable non-zero segmentation performance and was considered for Stage 2.}
...
\begin{figure}[htbp]
    ...
    \caption{Validation mask $\mathrm{mAP}_{50\text{:}95}$ for the successful unique Stage 2 capacity-tuning configurations. The best result was obtained with batch size 16 and image size 768, which was selected for Stage 3 refinement.}
...
Figures~\ref{fig:stage3_mask_map}, \ref{fig:stage3_val_seg_loss}, and
\ref{fig:stage3_lr_pg0} show the corrected Stage 3 comparison between refinement
from the strongest Stage 2 configuration (batch size 16, image size 768) and
refinement from the second-best unique Stage 2 configuration (batch size 16,
image size 640). The 768-pixel branch remains consistently stronger throughout
training, reaching approximately 0.63 mask $\mathrm{mAP}_{50\text{:}95}$,
whereas the 640-pixel branch stabilizes near 0.56. The corresponding validation
segmentation loss is also lower for the 768-pixel branch, while the
learning-rate curves are effectively identical. This indicates that the final
performance difference is explained by the quality of the inherited Stage 2
configuration rather than by a different optimization schedule.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.82\textwidth]{figures/results/stage3_yolo_mAP50-95__M.png}
    \caption{Stage 3 validation mask $\mathrm{mAP}_{50\text{:}95}$ for refinement from the best Stage 2 768-pixel configuration and the second-best unique 640-pixel configuration. The 768-pixel branch remains clearly superior throughout training.}
...
\begin{figure}[htbp]
    ...
    \caption{Stage 3 validation segmentation loss for the two corrected refinement branches. The branch initialized from the stronger Stage 2 configuration maintains the lower validation loss.}
    \label{fig:stage3_val_seg_loss}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.82\textwidth]{figures/results/stage3_yolo_lr_pg0.png}
    \caption{Stage 3 learning-rate schedule (\texttt{lr/pg0}) for the two corrected refinement branches. Both runs follow essentially the same decay schedule, so the difference in final accuracy cannot be attributed to different learning-rate behavior.}
    \label{fig:stage3_lr_pg0}
\end{figure}
...
University, Building 31. Table~\ref{tab:adc_original_test} shows that this
...
observed during live testing with the mobile cart in HAN University Building 31.
...
\begin{figure}[htbp]
    ...
    \end{minipage}
    \caption{End-to-end visualization of the final deployed \gls{ros2} perception output for a representative in-cabin elevator panel. Left: yellow predicted masks with red crosses marking the representative button centers used for depth projection, together with blue distance annotations. Right: green bounding boxes with recognized button labels and \gls{yolo} confidence scores. Together, the two views show how the final \texttt{/button\_targets\_3d\_ocr} output combines geometric localization and semantic interpretation.}
