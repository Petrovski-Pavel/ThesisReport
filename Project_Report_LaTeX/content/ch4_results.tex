...
target confidence (0.8422 vs.\ 0.8184). The meaningful distance statistics also
remained favorable: the mean valid-target distance stayed close to the baseline
(0.6776~m vs.\ 0.6975~m), while the median distance increased slightly
(0.5843~m vs.\ 0.5594~m) and the 90th-percentile distance also increased
slightly (0.8310~m vs.\ 0.7955~m). Maximum-distance values are not reported
here because the farthest outliers included implausible depth estimates that did
not correspond to real button observations. However, qualitative inspection of
matched replay frames still showed that some individual button instances were
recognized at substantially larger distances than with the baseline model, in
several cases by more than a factor of two. This replay result complements the
held-out test-set results by showing that \gls{adc}-based retraining improved
deployment robustness and confidence on the actual target environment, while
maintaining comparable overall distance performance and improving some
individual long-range detections.
