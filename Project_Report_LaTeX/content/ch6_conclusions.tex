...
\begin{enumerate}
    ...
    \item Environment-specific fine-tuning with \gls{adc} data from HAN University R29 produced the most practically valuable \gls{yolo} improvement, substantially increasing target-domain performance while preserving original-domain performance. In the controlled university replay experiment, the retrained model also produced more detected frames, more valid-depth frames, and a higher mean target confidence (0.8422 vs.\ 0.8184). Mean valid-target distance remained comparable to the baseline (0.6776~m vs.\ 0.6975~m), while the median and 90th-percentile distances increased slightly (0.5843~m vs.\ 0.5594~m and 0.8310~m vs.\ 0.7955~m, respectively), although several matched button instances were still observed at much larger distances than with the baseline model.
...
collection within a modular \gls{ros2} architecture.
