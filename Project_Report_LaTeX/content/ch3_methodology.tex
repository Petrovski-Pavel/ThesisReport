...
\texttt{elevator\_split}, which is prepared in YOLO-compatible
...
The initial dataset used for the baseline YOLOv8-Seg model was split as follows (\texttt{elevator\_split}):
\begin{itemize}
...
In the original dataset, annotations are used in the project-specific YOLO-compatible
...
converted to YOLO-segmentation labels with a dedicated conversion script.
This ensures compatibility with YOLO while maintaining a consistent train/val/test
...
\paragraph{OCR dataset construction.}
The recognition dataset was built from the original \texttt{train\_set} and
\texttt{test\_set} image collections together with their Pascal VOC XML
annotations. For each annotated button, the corresponding bounding box was cropped
from the source image and enlarged with a fixed contextual padding of 15\% of the
box width and height. This padding was included to retain relevant local context,
such as the button border or icon outline, while still presenting the recognizer
with a mostly isolated button instance.

The dataset builder grouped samples by source image before creating a fixed
80/20 train/validation split using a deterministic random seed. This means that
all button crops originating from one source image were placed either in the
training set or in the validation set, but never in both. This design avoids
optimistic validation scores caused by strong visual similarity between buttons
from the same panel. The final recognition
dataset used for the main OCR experiments, denoted
...
Several dataset variants were explored during development. In particular,
experiments were conducted with a dataset containing all labels, a dataset with
icon labels removed, and a dataset where generic free-text placeholder labels were
excluded. The final dataset definition removed labels named exactly
\texttt{text} and labels beginning with the prefix \texttt{text\_}. These labels
were not useful semantic targets for button interpretation, because they indicate
the presence of unspecified free text rather than the actual text string visible on
the button. Retaining them caused the recognizer to over-predict generic
``TEXT''-like outputs and reduced its ability to generalize to unseen textual
content. By excluding only these placeholder labels, the final dataset preserved
meaningful button semantics such as floor numbers, alphanumeric floor identifiers,
and icon-derived semantic labels including \texttt{OPEN}, \texttt{CLOSE},
\texttt{ALARM}, and \texttt{KEYHOLE}.
...
backbone suitable for embedded hardware. The architecture uses a
\texttt{PPLCNetV3} backbone with scale 0.95, followed by an \texttt{SVTR}-style
sequence modeling neck and a \texttt{MultiHead} recognition head combining
\texttt{CTC} and \texttt{NRTR} objectives. In practice, this architecture is
appropriate for the current task because button labels range from single digits and
single letters to short strings such as \texttt{10}, \texttt{4A}, or
\texttt{CLOSE}, while still requiring efficient inference on cropped inputs.

Only the recognition component of the OCR stack was used. The detection component
of PaddleOCR was intentionally omitted from the final recognition pipeline, because
...
dataset. Input crops were resized to an effective recognition shape of
$48 \times 320$ pixels. Training used the Adam optimizer with $\beta_1 = 0.9$,
$\beta_2 = 0.999$, cosine learning-rate scheduling, an initial learning rate of
$5 \times 10^{-4}$, and L2 regularization with weight decay
$3 \times 10^{-5}$. A five-epoch warmup was applied in the initial fine-tuning
configuration. The training data loader used a multiscale sampling strategy and a
batch size of 128, while recognition augmentation
(\texttt{RecConAug}, probability 0.5) was enabled to improve robustness to
...
\paragraph{Inference integration and post-processing.}
After training, the best OCR checkpoint was exported in Paddle inference format and
integrated into the button-interpretation pipeline. At runtime, the instance
segmentation model produces button crops, each crop is passed through the
recognition model, and the highest-confidence recognition output is selected. OCR
outputs are normalized to uppercase form and filtered through simple validity
constraints, including a maximum output length and a restricted character pattern
for accepted predictions.

For non-numeric predictions, an additional lightweight vocabulary-based correction
step was introduced. The predicted string is normalized and compared against a
precomputed list of valid non-numeric labels derived from the dataset annotations.
Fast bounded edit-distance matching is then used to map near-miss predictions such
as \texttt{OPE}, \texttt{ALAR}, or \texttt{BLOSE} to their most plausible target
labels when a unique close match exists. This post-processing step is computationally
cheap and improves the practical usability of the OCR module without requiring a
larger recognition model.
