...
This chapter interprets the results presented in Chapter~\ref{ch:results} and relates them to
the project objective of developing a scalable computer-vision
workflow for robot interaction with elevator infrastructure on embedded
hardware. Taken together, the implemented stages instantiate the main loop
outlined earlier in the workflow-and-architecture overview in
Figure~\ref{fig:scalable_workflow}: data collection, annotation support,
retraining, deployment, and environment-specific refinement.
...
detection, crop-based \gls{ocr}, and a modular \gls{ros} runtime pipeline. The
...
For \gls{yolo}, the baseline \gls{yolo}v8s segmentation model remained the most reliable
choice for deployment. Hyperparameter tuning improved validation performance, but
those gains did not translate into a better final held-out test model. A
plausible explanation is that the tuning process optimized more strongly for the
validation split, while the baseline configuration generalized slightly better
to unseen test images. Likewise, YOLO26-small did not outperform the baseline
under the same evaluation conditions. This is an important result because it shows that, for this task, a
...
\gls{adc} data from HAN Building R29, which substantially improved
...
At system level, the deployed \gls{ros} pipeline demonstrates that model
...
The results provide a coherent answer to the main research question. A
computer-vision workflow for hospital elevator-button perception can be designed
by using instance segmentation for button localization, crop-based \gls{ocr} for
semantic interpretation, \gls{tensorrt} export for embedded execution, and an
\gls{ros}-based split-node architecture for modular runtime integration.
...
class for multi-floor service-robot mobility. This focus was also aligned with
Ambyon's immediate deployment priorities, which made elevator-button perception a relevant and high-value interaction target for the present project.
This focus proved appropriate: the
...
domain. In the controlled R29 single-button experiment, the adapted detector
produced more frames with detections and longer typical valid-detection
distances once the baseline model's manually reviewed long-range false positives
were excluded from the valid-distance comparison. The reviewed deployment-style
university run further showed that accepted detection confidence degrades as
distance increases. This supports the decision to include an \gls{adc} in the
full workflow and suggests that further fine-tuning on carefully collected
deployment data is likely to remain a high-value improvement path.
...
single universal summary metric.
...
devices or other sensing configurations. In addition, the current Jetson setup
still runs from a microSD card rather than an SSD, which is not ideal for long
logging sessions, dataset export, or sustained deployment experiments.
...
Fifth, the current \gls{adc} closes the loop only partially. It can collect,
filter, and store samples in a format that is already useful for later \gls{yolo}
and \gls{ocr} updates, but there is still a human in the loop for verification.
In addition, export from the robot, transfer of the dataset session, integration
with the \gls{cvat} tooling, and redeployment of updated models are not yet
...
particularly important for educational and research robots such as CareBOTS, where
iterative development and subsystem replacement are expected.

A further practical deployment consideration is model licensing. The \gls{yolo}
framework used in this project is distributed by Ultralytics under the AGPL-3.0
open-source license, with separate commercial licensing options for enterprise
use. In contrast, PaddleOCR and Torchvision are distributed under the more
permissive Apache 2.0 and BSD-3-Clause licenses, respectively. Although
licensing did not affect the technical validity of the experiments reported
here, it should be considered in any future transition from research
prototyping to commercial or operational deployment.
