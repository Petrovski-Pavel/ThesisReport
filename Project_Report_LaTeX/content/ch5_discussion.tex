...
outlined earlier in Figure~\ref{fig:scalable_workflow}: data collection,
annotation support, retraining, deployment, and environment-specific refinement.
...
\gls{adc} data from HAN University R29, which substantially improved
...
\gls{ros2}-based split-node architecture for modular runtime integration.
...
Ambyon's immediate deployment priorities, which made elevator-button perception
the most relevant and highest-value interaction target for the present project.
...
complete autonomous hospital robot. The next decisive step is therefore not only
more perception benchmarking, but real robot testing in hospital-like or actual
hospital environments, where the perception output can be validated in the full
navigation-and-interaction loop.
